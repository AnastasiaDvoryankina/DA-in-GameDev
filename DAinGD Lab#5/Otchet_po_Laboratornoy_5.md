...
Этап 1. "Эксперимент" , 
Для начала произвольно и незначительно поменяем параметры и посмотрим, что из этого выйдет. Отталкиваться на этом этапе будем от имеющегося графика в tensorflow, а не от документации проекта `ml-agent's` или статей с википедии. Идея только одна: попробовть сломать все, что можно, а после выяснить, каким образом оно ломается и хорошо ли это в нашей задаче.
...
Некоторые выводы: сломалась кривая хаотичности системы и это отразилась на результате. Cumulative Reward - вознаграждение скачет, обычно она более прямая и предсказуемая.

Теперь попробуем поставить идею повышения скорости обучаемости модели. Т.е. я возвращаюсь к параметрам более близким к стандартным, но с уклоном в более быстрый процесс обучения. Для этого я приметила из документации несколько интересных парметров тренера(наш тип PPO).
...
Сразу же можно отметить более предсказуемый результат rewards и entropy, которая даже стала снижаться на позднем этапе обучения, когда система стала дорабатывать имеющиеся результаты до идеального состояния.
...
На диаграмме мы видим зависимость вознаграждения от совершенных действий. 
...
## Выводы
В ходе выпонения лабораторной работы мы познакомились с инструментом tensorflow - графическим отображением поведения нейросети во время обучения, открытая программной библиотекой для машинного обучения. Разработанная компанией Google для решения задач построения и тренировки нейронной сети с целью автоматического нахождения и классификации образов tensorflow, достигая качества человеческого восприятия. Применяется как для исследований, так и для разработки собственных продуктов Google. Основной API для работы с библиотекой реализован для Python, также существуют реализации для R, C Sharp, C++, Haskell, Java, Go и Swift.
Также мы повторно обратились к документации `ml-agent 19` для решения второго этапа нашей задачи. Это закрепило навыки, которые были полученны при исполнении лабораторной работы 3 и создании RollerAgent.
Для достижения идеального результата необходимо балансировать параметры тренера отдельно и поочередно, чтобы после шага назад мы могли вернуться к предыдущему этапу и пересмотреть стратегию дальнейшего обучения, отображать поведения сети мы можем с помощью tensorflow.
